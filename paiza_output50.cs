...
class Program
{
    static void Main()
    {
        ...
        int A = Int32.Parse(array1[2]);
        int B = Int32.Parse(array1[3]);
        string AA = A.ToString().PadLeft(9);
        string BB = B.ToString().PadLeft(9);

        // 1つの要素となる文字列を作成 例: "(1, 2)"
        string element = string.Format("({0}, {1})", AA, BB);
